...
function onBlur(){
   if(inputEl.value.length < inputEl.getAttribute('data-length')){ //условие - если значение атрибута data-length меньше значения inputEl.value.length(длины value)
    ...
    inputEl.classList.add('invalid'); //добавляем класс из списка классов элемента.
      } else { //если условие не выполнено и inputEl.value.length больше, то:
